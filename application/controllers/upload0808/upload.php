...
class Upload extends CI_Controller
{
	...
	public function upload_img()
	{
		...
		$thumbsdir = $useruploaddir.DS.'thumbs';
		if (!is_dir($thumbsdir)){
			mkdir($thumbsdir,777);
		}
		...
		$this->setPath_url_img_thumb_upload_folder(base_url() . 'uploads/'.$yearfolder.'/'.$monthfolder.'/'.$dayfolder.'/'.$useridss.'/thumbs');
		...
		if ($this->do_upload())
		{
			...
			$info->url = base_url() . "assets/img/articles/" . $name;
