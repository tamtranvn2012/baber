...
class Uploadbipost extends CI_Controller
{
    ...
    public function upload_img()
    {
        ...
        if($_FILES['userfile']['name'] == ''){
            ...
            $upidpost = $upid = $_REQUEST['upid'];
            $userid = $this->input->cookie('userid', TRUE);
            $upiddb = $this->profile_model->get_upid_by_userid($userid)[0]->upid;
           // var_dump($_REQUEST['addnewbipost']);exit;
            if(intval($upiddb) == intval($upid)){
                $babershopname = $_REQUEST['babershopname'];
                $baber_type = 'independent';
                $baber_name = $_REQUEST['babername'];
                $tags = $_REQUEST['tags'];

                $resultapid = $this->profile_model->get_apid_by_upid_upid($upid);
    ...
    // Function called by the form
    public function upload_image_register()
    {
        if($_FILES['userfile']['name'] == ''){
            $photolink = 0;
